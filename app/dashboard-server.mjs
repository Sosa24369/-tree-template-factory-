/**
 * dashboardApi() — a Vite DEV-SERVER plugin that gives the content dashboard a small
 * filesystem/git/image backend.
 *
 * WHY A PLUGIN AND NOT A SEPARATE SERVER: the brief says match the stack and add no
 * second framework. This is Vite middleware — the same dev server that already runs
 * the app — so there is nothing new to install or boot. It is registered ONLY in
 * `configureServer`, so it exists during `vite dev` and CANNOT be present in a built,
 * deployable bundle. The dashboard is local-only by construction.
 *
 * WHAT IT GUARANTEES:
 *  - Every write is confined to /clients/<slug>.json or app/public/assets/<slug>/.
 *    `slug` is validated against /^[a-z0-9-]+$/ and never joined from user input
 *    beyond that, so one client can NEVER write into another's folder (R4, made
 *    structural rather than merely discouraged).
 *  - Uploaded images run through the SAME sharp pipeline as scripts/optimize-assets +
 *    generate-srcset (WebP q80, cap 1600w, 400/800/1200w variants), so an unoptimised
 *    upload cannot silently blow the mobile performance budget.
 *  - Saving validates the P0 invariants server-side (phone E.164, thank-you not the
 *    leaked external domain, consent present, required fields non-blank) before it
 *    writes, then git-commits the record together with any new image files.
 */
...
export function dashboardApi() {
  return {
    ...
    configureServer(server) {
      const ROOT = join(server.config.root, '..'); // app/ -> repo root
      const CLIENTS = join(ROOT, 'clients');
      const PUB = join(server.config.root, 'public');
      ...
      server.middlewares.use(async (req, res, next) => {
        const url = req.url || '';
        if (!url.startsWith('/api/dash/')) return next();
