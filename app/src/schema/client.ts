...
export interface PhotoSet {
  ...
  src: string;
  /**
   * Stable identity for explicit slot assignment (see ClientRecord.photoSlots). Absent on
   * every record written before Phase 1b: `src` already carries a content hash, so
   * `id ?? src` is stable and no record needs rewriting. New uploads get one.
   */
  id?: string;
  /**
...
export interface ClientRecord {
   ...
   *  Since Phase 1b these are read as TAGS on one client library rather than as containers
   *  a template indexes by position — array order is still the auto-fill order. */
  photos: Partial<Record<'removal' | 'trimming' | 'storm' | 'generic', PhotoSet[]>>;

  /**
   * Explicit photo placement, per template: which photograph sits in which slot.
   * Optional and sparse — a slot that is absent is auto-filled by lib/placement.ts, which
   * reproduces the pre-Phase-1b behaviour exactly, so a record without this key renders
   * byte-identically. The studio writes it for one template the first time an assignment
   * on that template is changed; nothing migrates a record wholesale.
   *
   * Keyed `templateId` -> `slotId` (a one-cell slot) or `slotId.N`, 1-based (a multi-cell
   * slot). The value is a photo's `id ?? src`. Slots that consume the whole set (the
   * removal-a rail, the agnostic gallery) take no keys: their order IS the library order.
   */
  photoSlots?: Partial<Record<string, Record<string, string>>>;
