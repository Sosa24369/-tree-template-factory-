...
import { CallCta, Eyebrow, FORM_ANCHOR, Heading, Section, type Copy } from './shared';

export function Estimate({ client, copy }: { client: ResolvedClient; copy: Copy }) {
  const orCall = copy('estimate.orCall');

  return (
    <Section id={FORM_ANCHOR} tone="tint" className="st-estimate">
      <div className="st-estimate-grid">
        <div className="st-estimate-copy">
          <Eyebrow>{copy('estimate.eyebrow')}</Eyebrow>
          <Heading as="h2" className="st-h2" parts={[copy('estimate.h2a'), copy('estimate.h2b')]} />
          <SafeText as="p" className="st-lede" value={copy('estimate.lede')} />
