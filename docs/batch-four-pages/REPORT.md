...
| bundle, gzip | **+2,920 B of 3,072** (JS +3,007, CSS −87 vs live): the areas grid, the bar observer, the poster field, the two card entries and their waivers in the bundled record; 152 B of headroom |
| Lighthouse, August method, four pages | **J Valdez removal-a 99 / 1.70 s / 381,865 B** (before 98 / 1.85 s / 602,973); **J Valdez trimming-a 98 / 1.89 s / 87,834 B** (before 98 / 1.85 s / 87,834 — inside the run spread); **Texas Tree Tops removal-a 97 / 2.00 s / 405,003 B** (before 98 / 1.90 s / 441,941; gate 98 / 1.90 s / 386,105) — see the note below; **Texas Tree Tops storm-a 99 / 1.74 s / 13,990 B** (unchanged) |

**The one open trade-off — Texas Tree Tops removal-a.** After the sweep the page read 97 / 2.07 s / 479,505 B: the eager proof strip had taken a 112 KB photograph and the clip's poster added 29 KB. Recovered by weight (the 51 KB lift photo in the proof cell, the grinder in the deferred grid; the poster at 320 px, 15.7 KB): 97 / 2.00 s / 405,003 B — 37 KB under the pre-batch baseline, one point and ~100 ms behind it, and the whole remaining gap is the poster, an ordinary image request that cannot be lazy. Your call: (a) accept — a real frame on the rail's first tile for ~100 ms on a throttled mobile run; (b) a lazy poster set by an observer (~150 B of the 152 B bundle headroom); (c) drop the clip from the rail (the black tile was the sweep's defect). I recommend (a).
